...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    >
        @if (count($containers) || $emptyLabel !== false)
            <div class="relative table-repeater-container rounded-xl ring-1 ring-gray-950/5 dark:ring-white/20">
                <table class="w-full">
                    <thead @class([
                    ...
                    <tr class="text-xs md:divide-x md:divide-gray-950/5 dark:md:divide-white/20">
                        ...
                        @if ($hasActions && count($containers))
                            <th class="w-px p-2 bg-gray-100 table-repeater-header-column last:rounded-tr-xl dark:bg-gray-900/60">
                                <span class="sr-only">
                    ...
                    <tbody
                        ...
                        class="divide-y table-repeater-rows-wrapper divide-gray-950/5 dark:divide-white/20"
                    >
                    @if (count($containers))
                        @foreach ($containers as $uuid => $row)
                            ...
                            >
                                ...
                                @if ($hasActions)
                                    <td class="w-px p-2 table-repeater-column">
                                        <ul class="flex items-center px-2 table-repeater-row-actions gap-x-3">
                    ...
                    @else
                        <tr class="table-repeater-row table-repeater-empty-row">
                            <td colspan="{{ count($headers) + intval($hasActions) }}"
                                class="w-px p-4 italic text-center table-repeater-column table-repeater-empty-column">
                    ...
                    @endif
                     {{-- Add a summary row --}}
                    @if ($hasSummary)
                        <tr class="bg-gray-100 table-repeater-summary-row dark:bg-gray-800">
                            @foreach (collect($containers)->first()->getComponents() as $cell)
                                <td
                                    @class([
                                        'table-repeater-column p-2 text-sm',
                                    ])
                                >
                                    @if (in_array($cell->getName(), $summaryColumns))
                                        <span wire:key="{{ $statePath }}.summary.{{ $header->getLabel() }}" wire:model="data.{{$cell->getName()}}">
                                            {{ Number::currency($getSummaryTotal($cell->getName())) }}
                                        </span>
                                    @endif
                                </td>
                            @endforeach
